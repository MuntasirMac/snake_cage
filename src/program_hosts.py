...
def log_into_account():
    ...
    if not account:
        error_msg(f"ERROR: Could not find account with email: {email}")
        return
...
def register_cage():
    ...
    cage = svc.register_cage(state.active_account, name,
                allow_dangerous, has_toys, carpeted, meters, price
                )
    state.reload_account()
    success_msg(f"Registered New Cage With Cage Id: {cage.id}")
